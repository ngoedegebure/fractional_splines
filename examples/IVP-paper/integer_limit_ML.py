...
from scipy.special import gamma as gamm_func
...
def ML(z, alpha, beta=1.0, n_terms=50):
    ...
    numerator = z_arr[:, np.newaxis] ** k_vals
    denominator = gamm_func(alpha * k_vals + beta)
...
f = Lin_DE(params = params, bernstein = True, transpose=False)
...
if "pol" in PLOT_SELECTION:
    ...
    N = int(T / h) + 1
    t_knot_vals = (np.linspace(0, 1, N)) ** 1 * T
